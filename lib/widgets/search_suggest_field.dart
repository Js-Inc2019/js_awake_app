...
//   ・チップ = 入力の部分一致候補（最大 maxSuggestions 件）。一致部分を FieldTokens.accent で強調。
...
//   ・色は FIELD の既存 FieldTokens 定数のみ（accent=#6FD6B4 等・新規hex追加なし）。タップ領域44pt以上。
      ...
      text: TextSpan(
        ...
        children: [
          ...
          TextSpan(
            ...
            style: const TextStyle(color: FieldTokens.accent, fontWeight: FontWeight.bold),
          ),
  ...
  Widget build(BuildContext context) {
    ...
    return Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              return InkWell(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(20),
                    border: Border.all(color: FieldTokens.accent.withValues(alpha: 0.6)),
                  ),
      ...
      TextField(
        ...
        // 入力中文字は textBody ではっきり（背景に沈まない）。
        style: const TextStyle(color: FieldTokens.textBody, fontSize: 14),
        decoration: InputDecoration(
          isDense: true,
          filled: true,
          // 背景から浮かせる薄塗り: surface系に一段明るい既存定数が無いため、
          // アクセント色(accent=#6FD6B4)の 8% 薄塗りで代用（新規hexリテラルなし・案1準拠）。
          fillColor: FieldTokens.accent.withValues(alpha: 0.08),
          hintText: widget.hintText,
          // ヒントは textSupport より明るく（textBody の 60%）・fontSize 14。
          hintStyle: TextStyle(color: FieldTokens.textBody.withValues(alpha: 0.6), fontSize: 14),
          // 検索アイコンはアクセント色・サイズ20（16以上）。
          prefixIcon: const Icon(Icons.search, color: FieldTokens.accent, size: 20),
          suffixIcon: widget.controller.text.isEmpty
              ? null
              : IconButton(
                  icon: const Icon(Icons.clear, color: FieldTokens.textSupport, size: 18),
                  onPressed: () {
          ...
          enabledBorder: OutlineInputBorder(
            ...
            borderSide: const BorderSide(color: FieldTokens.accent, width: 1.5),
          ),
