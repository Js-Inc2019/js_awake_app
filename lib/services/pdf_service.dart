...
class PdfService {
  ...
  Future<File> generateDailyReport({
    ...
    // テナント名（会社名）を動的取得（固定テナント名 J's は埋め込まない）。
    // 取得元: prefs 'company_name'（ログイン時に auth_service.dart:136 で data['company_name'] を保存）。
        ...
        build: (ctx) => [
          ...
          pw.Container(
            ...
            child: pw.Row(
              ...
              children: [
                pw.Text(companyName.isNotEmpty ? '$companyName 日報' : '日報',
          ...
          pw.Row(
            mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
            children: [
              // 会社名 null/空は非表示（左は空プレースホルダでGenerated表記の右寄せを維持）。
